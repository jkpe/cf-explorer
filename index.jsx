...
const API_KEY_STORAGE = “cf_explorer_api_key”;
const ACCOUNT_ID_STORAGE = “cf_explorer_account_id”;

// ── Cloudflare API helpers ──────────────────────────────────────────
const cfFetch = async (accountId, apiKey, path, opts = {}) => {
...
.setup-subtitle { color: var(–text-dim); font-size: 13px; line-height: 1.5; }
...
.notice { padding: 10px 16px; background: rgba(249,115,22,0.06); border-bottom: 1px solid var(–border); font-size: 12px; color: var(–text-dim); display: flex; align-items: center; gap: 8px; }
`;
...
// ── KV Explorer ───────────────────────────────────────────────────────
function KVExplorer({ accountId, apiKey }) {
...
try {
const d = await cfFetch(accountId, apiKey, `/accounts/${accountId}/storage/kv/namespaces?per_page=100`);
...
const d = await cfFetch(accountId, apiKey, `/accounts/${accountId}/storage/kv/namespaces/${ns.id}/keys?${q}`);
setKeys(d.result || []);
setCursor(d.result_info?.cursor || null);
setPrevCursors(prev);
} catch (e) { setError(e.message); }
setLoading(false);
}, [accountId, apiKey, search]);

const selectNs = (ns) => { setSelectedNs(ns); setSelectedKey(null); setValuePaneOpen(false); setPrevCursors([]); loadKeys(ns, null, []); setSidebarOpen(false); };

const loadValue = async (key) => {
setSelectedKey(key); setValuePaneOpen(true); setLoadingVal(true);
try {
const res = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/storage/kv/namespaces/${selectedNs.id}/values/${encodeURIComponent(key.name)}`, { headers: { Authorization: `Bearer ${apiKey}` } });
...
// ── D1 Explorer ───────────────────────────────────────────────────────
function D1Explorer({ accountId, apiKey }) {
...
setLoading(true); setError(””);
try {
const d = await cfFetch(accountId, apiKey, `/accounts/${accountId}/d1/database?per_page=100`);
...
try {
const d = await cfFetch(accountId, apiKey, `/accounts/${accountId}/d1/database/${selectedDb.uuid}/query`, {
...
const [accountId, setAccountId] = useState(localStorage.getItem(ACCOUNT_ID_STORAGE) || “”);
const [apiKey, setApiKey] = useState(localStorage.getItem(API_KEY_STORAGE) || “”);

const save = () => {
if (!accountId.trim() || !apiKey.trim()) { toast(“Fill in both fields”); return; }
localStorage.setItem(ACCOUNT_ID_STORAGE, accountId);
localStorage.setItem(API_KEY_STORAGE, apiKey);
onSave(accountId, apiKey);
...
<div>
<div className="setup-title">Cloudflare <span style={{ color: “var(–accent)” }}>Explorer</span></div>
<div className=“setup-subtitle” style={{ marginTop: 6 }}>Connect your Cloudflare account to explore KV namespaces and D1 databases.</div>
</div>
<div className="field">
<label>Account ID</label>
<input value={accountId} onChange={e => setAccountId(e.target.value)} placeholder=“a1b2c3d4e5f6…” />
</div>
<div className="field">
<label>API Token</label>
<input type=“password” value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder=“Bearer token with KV+D1 read” />
</div>
<div className=“notice” style={{ borderRadius: “var(–radius)”, border: “1px solid var(–border)” }}>
...
const a = localStorage.getItem(ACCOUNT_ID_STORAGE);
const k = localStorage.getItem(API_KEY_STORAGE);
return a && k ? { accountId: a, apiKey: k } : null;
});
...
{creds && <button className="btn btn-ghost btn-sm" onClick={clear}><Icon name="settings" size={13} /> Reset</button>}
</div>
</div>

```
    {!creds ? (
      <SetupPanel onSave={(a, k) => setCreds({ accountId: a, apiKey: k })} />
    ) : (
      <>
        <div className="tabs">
        ...
        <div className="content">
          <div className={`panel ${tab === "kv" ? "active" : ""}`}>
            <KVExplorer accountId={creds.accountId} apiKey={creds.apiKey} />
          </div>
          <div className={`panel ${tab === "d1" ? "active" : ""}`}>
            <D1Explorer accountId={creds.accountId} apiKey={creds.apiKey} />
          </div>
        </div>
      </>
    )}
